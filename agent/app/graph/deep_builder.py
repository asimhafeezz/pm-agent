...
# Use build_graph(services, execution_mode='quick', deep_analysis=True) instead.
...
def _deep_route_by_intent(state: AgentState) -> str:
    """Route based on intent — deep analysis always goes through full pipeline."""
...
def _make_fetch_deep_data_node(services: GraphServices):
    ...
    async def _node(state: AgentState) -> Dict[str, Any]:
        ...
        for result in results:
            if isinstance(result, Exception):
                warnings.append(f'deep_fetch_error: {result}')
...
def build_deep_graph(services: GraphServices):
    """Build the deep analysis LangGraph pipeline."""
    ...
    # --- Deep analysis: parallel fetch (all data + web research) ---
    graph.add_node('fetch_deep_data', _make_fetch_deep_data_node(services))
    ...
    graph.add_conditional_edges('parse_intent', _deep_route_by_intent, {
        'synthesize_report': 'synthesize_report',
        'build_candidates': 'build_candidates',
    })

    # Data pipeline: candidates → resolve → parallel deep fetch
    graph.add_edge('build_candidates', 'resolve_symbols')
    graph.add_edge('resolve_symbols', 'fetch_deep_data')

    # After deep fetch: classify news → deep scoring → report
    graph.add_edge('fetch_deep_data', 'classify_news')
